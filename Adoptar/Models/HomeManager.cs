...
{   
    public class HomeManager
    {
        public void Insertar (Ficha ficha)
        {
            ...
            sentencia.CommandText = "Insert into Fichas (Fecha, Tipo, Titulo, Texto, Imagen, Autor, Sexo, Ubicacion, Edad, imagenFile) VALUES (GETDATE(), @Tipo, @Titulo, @Texto, @Imagen, @Autor, @Sexo, @Ubicacion, @Edad, @imagenFile)";
            ...
            sentencia.Parameters.AddWithValue("@imagenFile", ficha.Imagenfile);
        ...
        public List<Ficha> ConsultarTodos()
        {
            ...
            //SqlConnection conexion = new SqlConnection(
            //WebConfigurationManager.ConnectionStrings["conexion"].ConnectionString);
        ...
        public long ConsultarUltimoID()
        {
            ...
            conexion.Open();
            SqlCommand sentencia = conexion.CreateCommand();
            sentencia.CommandText = "SELECT TOP 1 ID FROM Fichas ORDER BY ID DESC;";
            SqlDataReader reader = sentencia.ExecuteReader();
            reader.Read();

            long fichaid = (int)reader["Id"];
            conexion.Close();
        ...
        public Ficha Consultar(long id)
        {
            ...
            ficha.Texto = (string)reader["Texto"];
            ficha.Imagen = reader["Imagen"].ToString();
            ficha.Imagenfile = reader["Imagenfile"].ToString();
            ficha.ID = (int)reader["Id"];
            ficha.Sexo = (string)reader["Sexo"];
            ficha.Ubicacion = (string)reader["Ubicacion"];
            ficha.Edad = (string)reader["Edad"];
